...
// - Always uses forward slashes (POSIX-style), even on Windows
//
...
func NormalizePath(p string) string {
	...
	// Use path.Clean (POSIX) instead of filepath.Clean (OS-specific)
	// because AGFS uses virtual filesystem paths with forward slashes.
	p = path.Clean(p)
...
func NormalizeS3Key(p string) string {
	...
	// Clean the path (use path.Clean for POSIX-style paths)
	p = path.Clean(p)
